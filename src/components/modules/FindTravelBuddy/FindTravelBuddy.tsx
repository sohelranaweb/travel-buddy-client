...
//   // Check for pending connection after login
//   useEffect(() => {
//     if (isAuthenticated && !isCheckingAuth && typeof window !== "undefined") {
//       const pending = localStorage.getItem("pendingConnection");

//       if (pending) {
//         try {
//           const { travelPlanId, destination, travelerName } =
//             JSON.parse(pending);

//           // Find the plan and auto-open modal
//           const plan = plans.find((p) => p.id === travelPlanId);
//           if (plan) {
//             setSelectedPlan(plan);
//             toast.success(
//               `Welcome back! You can now connect with ${travelerName} for ${destination}`
//             );
//           }
...
//       }
//     }
//   }, [isAuthenticated, isCheckingAuth, plans]);
...
//   const openModal = async (plan: TravelPlan) => {
...
//     if (!accessToken) {
//       // Save plan info for after login
...
//             travelerName: plan.traveler.name,
//           })
//         );
//       }

//       toast.info("Please login to connect with travelers");
//       router.push("/login?callbackUrl=/find-buddy");
//       return;
//     }

//     // User is logged in, open modal normally
//     setSelectedPlan(plan);
//   };
...
//       if (!accessToken) {
//         throw new Error("Please login to send buddy request");
//       }
...
//       closeModal();
//     } catch (error: any) {
//       const errorMessage = error.message || "Failed to send buddy request";
...
//                         src={plan.traveler.profilePhoto}
//                         alt={plan.traveler.name}
//                         className="w-16 h-16 rounded-full object-cover border-4 cursor-pointer hover:opacity-90 transition-opacity"
//                         style={{ borderColor: "var(--primary)" }}
//                       />
//                     </Link>
//                     <div className="flex-1">
